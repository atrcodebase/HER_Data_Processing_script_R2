...
tabs <- c("data", "Injuries", "Incidents", "Fatalities", "Section_B6_Other_Group")

## Filter empty rows
translation_log_filtered <- bind_rows(
  translation_log %>%
    mutate(old_value="", Translation_log="main") %>%
    select(KEY=KEY_Unique, Tool, Tab_Name, question=Question, old_value, new_value=Translation, Translation_log),
  translation_log2 %>%
    # Removing cases that were translated but should not be applied because the audio was removed during cleaning
    filter(`Should it apply?` %notin% "No") %>% 
    mutate(Tab_Name = "data", Translation_log="secondary",
           Translation = case_when(
             !is.na(`Final Translation`) & `Final Translation` %notin% c("..", "--", "-") ~ `Final Translation`,
             TRUE ~ Translation
           )) %>% # Double-check with QA
    select(KEY, Tool, Tab_Name, question=Question, new_value=Translation, Translation_log)
) %>% 
  # Fixing some of the Translation log issues
  mutate(
    question = case_when(
      question %in% "ANC_PNC_Why_Did_Not_Go_To_A_Health_Facility_For_An_Antenatal_Visit" ~ "ANC_PNC_Why_Not_Go_To_A_Health_Facility_For_An_Antenatal_Visit_Translation",
      question %in% "ANC_PNC_Why_Not_Satisfied_B4"~"ANC_PNC_Why_Not_Satisfied__B4_Translation",
      question %in% "ANC_PNC_Why_Chose_A_Different_Health_Facility_B4_1"~"ANC_PNC_Why_Chose_A_Different_Health_Facility__B4_1_Translation",
      question %in% "Nature_of_Complaint_Please_Elaborate_On_Your_Response"~"Nature_of_Complaint_Please_Elaborate_On_Your_Response_Description",
      question %in% "Please_Explain_Why_You_Were_Not_Satisfied_With_The_Resolution"~"Please_Explain_Why_You_Were_Not_Satisfied_With_The_Resolution_Description",
      paste0(question, "_Translation") %in% names(t2_data) & Tool %in% "Tool 2" ~ paste0(question, "_Translation"),
      paste0(question, "_Translation") %in% names(t3_data) & Tool %in% "Tool 3" ~ paste0(question, "_Translation"),
      TRUE ~ question),
    Tool = case_when(
      is.na(Tool) & question %in% names(t2_data) ~ "Tool 2",
      is.na(Tool) & question %in% names(t3_data) ~ "Tool 3",
      TRUE ~ Tool
    ),
    Tab_Name = case_when(
      is.na(Tab_Name) & question %in% c(names(t2_data), names(t3_data)) ~ "data",
      # is.na(Tab_Name) & question %in% names(t3_data) ~ "data",
      TRUE ~ Tab_Name
    )
  ) %>% unique()
